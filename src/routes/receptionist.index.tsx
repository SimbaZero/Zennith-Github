...
function ReceptionDashboard() {
  ...
  const notifyNext = () => {
    ...
    enqueueNotification({
      title: `Next patient: ${next.patient}`,
      body: `Hi ${next.patient} — you're next for ${next.doctor}. Please prepare to enter the clinical room.`,
    });
    const msg = willSendNow
      ? `SMS sent to ${next.patient}.`
      : `Queued for ${next.patient} — outside 07:00–20:00 window; will send at ${scheduled.toLocaleTimeString("en-ZA", { hour: "2-digit", minute: "2-digit" })}.`;
  ...
  return (
    <AppShell role="receptionist" title="Reception Dashboard" showBack={false}>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Stat label="TODAY'S APPOINTMENTS" value={String(appointments.length)} sub="Scheduled" />
        <Stat label="ACUTE QUEUE" value={String(queue.length)} sub="Walk-ins waiting" />
        <Stat label="LAST UPDATED" value={now.toLocaleTimeString("en-ZA", { hour: "2-digit", minute: "2-digit" })} sub="Auto-refresh" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl border p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={notifyNext} className="flex items-center gap-1.5 bg-[oklch(0.18_0.06_260)] text-white text-xs px-3 py-1.5 rounded-md hover:bg-[oklch(0.25_0.08_260)]">
              <MessageCircle size={12} /> Notify next via SMS
            </button>
          </div>
          <p className="text-xs text-muted-foreground mb-3">Send window: 07:00 – 20:00. Late messages are queued.</p>
          <ul className="space-y-1.5">
            {queue.length === 0 ? (
              <li className="text-sm text-muted-foreground py-4 text-center">Queue is empty</li>
            ) : queue.map((a, i) => (
              <li key={i} className={`flex items-center gap-3 p-2 rounded-md ${i === 0 ? "bg-[oklch(0.97_0.03_245)] border border-[oklch(0.85_0.08_245)]" : "hover:bg-secondary/40"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? "bg-[oklch(0.55_0.18_245)] text-white" : "bg-secondary"}`}>{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm truncate">{a.patient}</p>
                  <p className="text-[11px] text-muted-foreground">{a.doctor} · {a.type}</p>
                </div>
                <button onClick={() => remove(i)} className="text-xs text-muted-foreground hover:text-destructive">Remove</button>
              </li>
            ))}
          </ul>
          ...
          {log.length > 0 && (
            <div className="mt-4 pt-4 border-t">
              <p className="text-[10px] tracking-wider text-muted-foreground mb-2">RECENT SMS ACTIVITY</p>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border p-5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {appointments.map((a) => (
                  <tr key={a.time + a.patient} className="border-b last:border-0">
                    <td className="py-2.5 font-mono">{a.time}</td>
                    <td className="py-2.5">{a.patient}</td>
                    <td className="py-2.5 text-muted-foreground">{a.doctor.replace("Dr. ", "")}</td>
