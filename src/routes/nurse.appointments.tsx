...
import {
  useAppointments, useNow, computeEffective, setAppointmentStatus, addAppointment, highRiskPatients,
} from "@/lib/store";
...
function NurseAppointments() {
  ...
  const [draft, setDraft] = useState({
    time: "09:00", patient: "", doctor: "Dr. Mutizwa", type: "Follow-up", pid: "", note: "",
  });
  ...
  const update = (time: string, patient: string, status: AppointmentStatus) => {
    setAppointmentStatus(time, patient, status);
      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {effective.map((a) => (
                <tr key={`${a.time}-${a.patient}`} className="border-t hover:bg-secondary/40">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {actions.map(({ status, icon: Icon, cls }) => {
                        ...
                        return (
                          <button
                            key={status}
                            onClick={() => update(a.time, a.patient, status)}
                            title={status}
